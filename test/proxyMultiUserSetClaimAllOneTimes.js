...
contract('PhxAllocTest', function (accounts) {
    ...
    it('Set user locked phx info', async function () {
        ...
        let accArray = [accounts[0],accounts[1],accounts[2],accounts[3],accounts[4],accounts[5],accounts[6]];
        let amountArray = [amount,amount,amount,amount,amount,amount,amount];
        let startTimeArray = [startTime,startTime,startTime,startTime,startTime,startTime,startTime];
        let intervalArray = [interval,interval,interval,interval,interval,interval,interval];
        let allocTimeArray = [allocTimes,allocTimes,allocTimes,allocTimes,allocTimes,allocTimes,allocTimes];

        let msgData = unLockerInst.contract.methods.setMultiUsersPhxUnlockInfo(accArray,amountArray,startTimeArray,intervalArray,allocTimeArray).encodeABI();
        ...
        let res = await testViolation("multiSig setUserPhxUnlockInfo: This tx is not aprroved",async function(){
            await unLockerInst.setMultiUsersPhxUnlockInfo(accArray,amountArray,startTimeArray,intervalArray,allocTimeArray,{from:accounts[9]});
        });
        ...
        startTimeArray = [startTime,startTime,startTime,startTime,startTime,startTime,startTime];

        msgData = unLockerInst.contract.methods.setMultiUsersPhxUnlockInfo(accArray,amountArray,startTimeArray,intervalArray,allocTimeArray).encodeABI();
